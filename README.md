...
```
Front end(ambil username): {{ Auth::user()->username }} 
```

### Symbolic link (assets folder for user)
```
make a `storage folder` inside `public` folder
php artisan storage:link
```

### List Package (Composer Package)
```
...
### Something
```
Do `config:cache` after `composer update`
Create `symbolic link` to make new folder after regist
Change `upload_max_filesize` to `3M` at `php.ini` at xampp (tentative) 
```
